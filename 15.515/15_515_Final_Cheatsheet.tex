...
\subsubsection{Statement of Cash Flows: Three Types of Flows}
\begin{itemize}
    \item \Red{Operating Activities} Ongoing, regular, core business activities (e.g., cash from sale of goods and services, payment for inventory, salary and wage payments, income tax payments).
    \item \Red{Investment Activities} Acquiring and selling productive assets (e.g., purchase/sale of physical assets, investment/sales in securities, capital expenditures, other firms' stock/bonds).
    \item \Red{Financing Activities} Cash flows from activities related to external sources of financing (transactions including debt, equity, and dividends, e.g., dividend payments, stock or bond issuing, stock or bond repurchasing).
\end{itemize}

\Blue{Indirect Method:} required for reporting the operating section of the statement of cash flows. Reconciles Net Income (NI) from Operating Cash Flows (CFO) and provides insight into the quality of Net Income.
\Hint{In the initial stages of a firm's life, financing cash flows are often positive and investing cash flows are often negative.}

%\Blue{Accounting Guidelines for Contingencies:} A contingency expense must be recognized if the event is \emph{probable} and if the cost of the event can be \emph{reasonably estimated}.

%\Blue{PP\&E and Receivables Relationships:}
%\begin{itemize}
%    \item $\text{Beginning Net PP\&E} - \text{Depreciation} - \text{Write Offs} + \text{PP\&E Purchases} = \text{Ending Net PP\&E}$
%    \item $\text{Beginning A/R} - \text{Write Offs} + \text{Additional Credit Sales} - \text{Cash Collected} = \text{Ending A/R}$
%\end{itemize}

\Hint{Major Differences Between CFO and NI:}
\begin{itemize}
    \item \Green{Depreciation} More PPE (gross) $\rightarrow$ more depreciation $\rightarrow$ larger wedge between CFO and NI.
    \item \Green{Stock-based Compensation} Wages (Expense) paid against contributed capital $\rightarrow$ no cash outflow when wages are paid in options.
    \item \Green{Changes in Working Capital} Current assets are subtracted (Cash, A/R, Inventory, Prepaid Assets), current liabilities are added (A/P, Accrued Liabilities, and Unearned Revenue).
    \item \Green{If a company buys inventory}, no impact on NI but it reduces CFO. The same applies to unearned revenue, which affects CFO but not NI.
\end{itemize}

%\Red{CFO = NI - Accruals}
%\begin{itemize}
%    \item \textbf{+ non-cash expenses:} Expenses that reduce NI but did not require cash (e.g., depreciation expense).
%    \item Increases in non-cash current assets are \textbf{deducted} from NI to calculate Operating Cash Flow.
%    \item Increases in current liabilities are \textbf{added} to NI to calculate Operating Cash Flow.
%    \item $\pm$ gains/losses associated with investing activities (e.g., Property, Plant, and Equipment disposal).
%    \item $\pm$ changes in working capital accounts (Working Capital: current assets and liabilities).
%\end{itemize}
...
\Red{Bond Accounting}
\begin{itemize}
    \item \Blue{Par Value} (aka. face value) amount that is returned to the investor when the bond matures (or ``principal"). E.g. if a bond is bought at issuance for \$1,000, the investor bought the bond at its par value. Will get back the \$1,000 a maturity.
...
\includegraphics[width=0.9\columnwidth]{./Tax.png}
\end{multicols}
