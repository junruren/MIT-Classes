...
\Blue{Marginal Revenue (MR)}

\begin{multicols}{2}
$MR = \frac{\Delta Rev(Q)}{\Delta Q} = P(Q) + Q \frac{\Delta P(Q)}{\Delta Q}$

In order to sell more units, the price must be lowered for all units.  Decrease price results in:
\textbf{\textcolor{Green}{gain}} in Rev from Q increases + \textbf{\textcolor{Red}{loss}} in Revenue from P reduction
for all units.
...
\begin{itemize}
    ...
    \item \Blue{Positive ext.} benefits born by society; \eg education, vaccines, R\&D (ideas). External benefits (positive ext.) $\Rightarrow$ firms produce too little. \Green{Sol.} subsidize \underline{customers} the externaltiy amount (to shift \textbf{up the \underline{demand curve}}) \textit{Note:} \Hint{who gets subsidized (consumer vs producer) \underline{doesn't change the equilibrium quantity}. Effective price consumers pay is the same.}
\end{itemize}
...
\begin{enumerate}
    \item For Player 1: In each column, mark the largest payoff for Player 1.
    \item For Player 2: In each row, mark the largest payoff for Player 2.
    \item A cell is a Nash equilibrium if both payoffs in that cell are marked.
    \item \textbf{Verify:} For each candidate cell, confirm that:
        $\begin{cases}
            P_1(\text{row}) &\geq P_1(\text{other rows}) \\
            P_2(\text{col}) &\geq P_2(\text{other cols})
        \end{cases}$
\end{enumerate}
...
\Red{Antitrust Laws} regulate market competition, focusing on practices that tend to reduce rivalry between firms (e.g., price fixing and horizontal mergers) as well as those that can foreclose competition, furthering a dominant firm's position (e.g., vertical mergers and exclusive dealing). Price fixing is \textit{per se} illegal, and punishments typically include time in jail. \Green{\Eg} \begin{itemize}
    \item \Green{Sherman Act (1890)} Prohibits contracts, combinations, or conspiracies that restrain trade or commerce; monopolization == felony
    \item \Green{Clayton Act (1914)} Prohibits price discrimination, anti-competitive tying/exclusive deals, and mergers that lessen competition or create monopolies.
    \item \Green{Federal Trade Commission Act (1914)} Prohibits unfair competition and deceptive practices affecting commerce.
\end{itemize}
...
\Red{Cournot (quantity competition) vs Bertrand (price competition)} Price is lower (= MC) under Bertrand, resulting in greater CS thanl Cournot.

\Red{Stackelberg Model} A sequential game where the leader chooses its output first, and the the follower chooses its output after observing the leader's decision.
\Blue{Key Idea} The leader commits to a production level, anticipating the follower's best response.
\Blue{Assumptions} Both firms produce homogeneous goods and face the same market demand.
\Blue{Result} The leader often earns higher profits by influencing the follower's behavior; total output exceeds Cournot equilibrium output.
