...
\Red{Bertrand model/trap of price competition} Each firm's best response is to undercut its rival $\Rightarrow$ N.E. = price = MC (\ie a price war). \Green{Sol.} \Blue{product differentiation} when consumers have heterogeneous preferences; can be both \Hint{horizontal} (differentiated by quality; \eg Coke vs Pepsi) and \Hint{vertical} (differentiated by price; \eg faster vs slower computers).
