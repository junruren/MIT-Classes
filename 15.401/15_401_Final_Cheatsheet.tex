...
\Red{CAPM implication} investors are \Hint{only compensated for exposure to systematic risk}.

CAPM implies that we can measure systematic risk using $\beta$.

\Blue{Estimate $\beta$} as the slope coefficient when running this regression equation:
...
\begin{itemize}
    \item We usually estimate firm-level \underline{equity betas} ($\beta_E$) using the stock returns of the firm.
